...
select reportID, reportDate, clgCode, clgName, uCode, uName, reporterID, reporter, tel, Email,
	title, reportTag, descTxt
from userReport
where reportID = '201107090033'

select fileGUID36, oldFilename, extFileName 
from userReportAppendFile
where reportID = '201107090033'
select a.descTxt, u.* from userReport u left join userReportAnswer a on u.reportID=a.reportID
...
CREATE TABLE userReport(
	...
	clgName nvarchar(30) not null,	--院部名称:冗余字段，为了保存历史数据
	uCode varchar(8) null,			--使用单位代码
	uName nvarchar(30) null,		--使用单位名称:冗余字段，为了保存历史数据
...
CREATE NONCLUSTERED INDEX [IX_userReport1] ON [dbo].[userReport] 
(
	[clgCode] ASC,
	[uCode] ASC
)WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF) ON [PRIMARY]
...
AS
	...
	--取维护人的姓名：
	declare @clgName nvarchar(30), @uName nvarchar(30)		--报告人所在院部名称，使用单位名称
	declare @reporter nvarchar(30)		--报告人姓名
	select @clgName = clgName from college where clgCode = @clgCode
	select @uName = uName from useUnit where uCode = @uCode
	select @reporter = cName from userInfo where jobNumber = @reporterID

	set @reportDate = getdate()
	insert userReport(reportID, reportDate, clgCode, clgName, uCode, uName,
						reporterID, reporter, tel, Email, title, reportTag, descTxt)
	values (@reportID, @reportDate, @clgCode, @clgName, @uCode, @uName,
			@reporterID, @reporter, @tel, @Email, @title, @reportTag, @descTxt)
...
AS
	...
	begin tran
		insert userReportAnswer(reportID, answerDate, clgCode, clgName, uCode, uName,
								answerID, answer, answerTag, descTxt)
		select @reportID, @answerDate, clgCode, clgName, uCode, uName,
				jobNumber, cName, @answerTag, @descTxt 
		from userInfo
		where jobNumber = @answerID
		if @@ERROR <> 0 
		begin
			rollback tran
			return
