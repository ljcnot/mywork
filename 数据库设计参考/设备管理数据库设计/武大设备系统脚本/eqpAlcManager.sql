...
--现用户同意改成主从表结构，调拨增加时间
--单位合并的时候是通过调拨划拨设备
alter TABLE equipmentAllocation add	oprManID varchar(10) null			--调出经办人工号
alter TABLE equipmentAllocation add	oprMan varchar(30) null			--调出经办人姓名
...
CREATE TABLE equipmentAllocation(
	...
	--调出经办人：add by lw 2013-5-16为了统一权限操作而增加的一个字段
	oprManID varchar(10) null,			--调出经办人工号
	oprMan varchar(30) null,			--调出经办人姓名
...
AS
	...
	begin tran
		insert equipmentAllocation(alcNum, alcDate,
								oldClgCode, oldClgName, oldClgManager, oldUCode, oldUName,oprManID, oprMan,
...
AS
	...
	begin tran
		...
		set alcDate = @alcDate,
			...
			oprManID = @oprManID, oprMan = @oprMan,
...
/*
	...
	output: 
				@updateTime smalldatetime output,--更新时间
				@Ret		int output		--操作成功标识
							0:成功，1：要更新的调拨单息正被别人锁定，2:该单据已经通过审核（生效），不允许修改，
							5：该调拨单中的设备正在清查，
							9：未知错误
	author:		卢苇
	CreateDate:	2010-11-29
	UpdateDate: modi by lw 2011-10-15增加设备生命周期表中的变动凭证登记！
				...
				modi by lw 2012-12-25修订了写入设备生命周期表中的说明。
				modi by lw 2013-5-27检查是否有叠加的设备清查事务锁！
*/
