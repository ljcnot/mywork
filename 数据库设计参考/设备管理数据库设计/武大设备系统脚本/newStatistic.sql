...
/*
	...
	author:		卢苇
	CreateDate:	2011-2-8
	UpdateDate: 2012-10-30 by lw 修改模板，支持多种约束条件，简化存储结构，增加where子句静态化，方便快速查询明细数据。
*/
...
CREATE TABLE [dbo].[statisticTemplate](
	...
	templateType smallint default(0),	--模板类别：0->设备分类汇总，1->单位分组汇总，2->金额分段汇总,3->使用方向，4->经费来源
	templateName nvarchar(30) not null,	--统计模板名称
	templateDesc nvarchar(300) null,	--统计模板的描述
	publish int default(0),				--是否发布：0->未发布，1->发布；发布的统计指标自动出现在导航栏中
	
	--约束条件：
	typeCodeIsUsed int default(0),	--是否使用设备分类约束条件：0->不使用，1->使用静态定义的设备分类约束条件，2->运行时决定
	typeCodeLimite xml null,		--静态定义的设备分类约束条件，
									--使用xml存储的指标：N'<root>
														--	<row id="1">
														...
														--	...
														--</root>'
	moneySectionUsed int default(0),	--是否使用金额约束条件：0->不使用，1->使用静态定义的金额约束条件，2->运行时决定
	moneySectionLimiteOpr1 varchar(2) null,	--金额约束条件的比较运算符1
	moneySectionLimiteData1 numeric(12,2) null,	--金额约束条件的比较值1
	moneySectionLimiteOpr2 varchar(2) null,	--金额约束条件的比较运算符2
	moneySectionLimiteData2 numeric(12,2) null,	--金额约束条件的比较值2

	appDirUsed int default(0),	--是否使用使用方向约束条件：0->不使用，1->使用静态定义的使用方向约束条件，2->运行时决定
	appDirLimite xml null,		--静态定义的使用方向约束条件，
								--使用xml存储的指标：N'<root>
													--	<row id="1">
													--		<aCode>1</aCode>
													--		<aName>教学</aName>
													...
													--	...
													--</root>'
	fundSrcUsed int default(0),	--是否使用经费来源约束条件：0->不使用，1->使用静态定义的经费来源约束条件，2->运行时决定
	fundSrcLimite xml null,		--静态定义的经费来源约束条件，
								--使用xml存储的指标：N'<root>
														--	<row id="1">
														--		<fCode>1</fCode>
														--		<fName>教育事业费</fName>
	...
	--统计指标：
	useMethod int default(0),	--使用指标的方式：0->不使用，1->静态使用，具体分类见指标字段，2->运行时决定
	typeCodeQuota xml null,			--静态定义的设备分类统计指标，
									--使用xml存储的指标：N'<root>
														--	<row id="1">
														--		<eTypeCode>05010549</eTypeCode>
														--		<eTypeName>激光打印机</eTypeName>
														...
														--	...
														--</root>'
	particleSize int default(1),	--分组统计指标:1->院部，2->使用单位
	moneySectionQuota xml null,		--静态定义的金额分段统计指标，
									--使用xml存储的指标：N'<root>
														--	<row id="1">
														...
														--	...
														--</root>'
	appDirQuota xml null,		--静态定义的使用方向统计指标，
								--使用xml存储的指标：N'<root>
													--	<row id="1">
													--		<aCode>1</aCode>
													--		<aName>教学</aName>
													...
													--	...
													--</root>'
	fundSrcQuota xml null,		--静态定义的经费来源统计指标，
								--使用xml存储的指标：N'<root>
														--	<row id="1">
														--		<fCode>1</fCode>
														--		<fName>教育事业费</fName>
...
/*
	...
	input: 
				@templateID smallint,			--模板ID：如果是创建模板的时候检查，则填0；
	...
	author:		卢苇
	CreateDate:	2011-2-9
	UpdateDate: modi by lw 2012-10-31增加修改模板的时候检查名称处理
*/
...
/*
	...
	function:	1.添加统计模板
				注意：本存储过程锁定编辑！
	input: 
				@templateType smallint,		--模板类别：0->设备分类汇总，1->单位分组汇总，2->金额分段汇总,3->使用方向，4->经费来源
				@templateName nvarchar(30),	--统计模板名称
				@templateDesc nvarchar(300),--统计模板的描述
				
				--约束条件：
				@typeCodeIsUsed int,	--是否使用设备分类约束条件：0->不使用，1->使用静态定义的设备分类约束条件，2->运行时决定
				@typeCodeLimite xml,	--静态定义的设备分类约束条件，
										--使用xml存储的指标：N'<root>
															--	<row id="1">
															--		<eTypeCode>05010549</eTypeCode>
															--		<eTypeName>激光打印机</eTypeName>
															...
															--	...
															--</root>'
				@moneySectionUsed int,	--是否使用金额约束条件：0->不使用，1->使用静态定义的金额约束条件，2->运行时决定
				@moneySectionLimiteOpr1 varchar(2),	--金额约束条件的比较运算符1
				@moneySectionLimiteData1 numeric(12,2),	--金额约束条件的比较值1
				@moneySectionLimiteOpr2 varchar(2),	--金额约束条件的比较运算符2
				@moneySectionLimiteData2 numeric(12,2),	--金额约束条件的比较值2

				@appDirUsed int,	--是否使用使用方向约束条件：0->不使用，1->使用静态定义的使用方向约束条件，2->运行时决定
				@appDirLimite xml,	--静态定义的使用方向约束条件，
									--使用xml存储的指标：N'<root>
														--	<row id="1">
														--		<aCode>1</aCode>
														--		<aName>教学</aName>
														...
														--	...
														--</root>'
				@fundSrcUsed int,	--是否使用经费来源约束条件：0->不使用，1->使用静态定义的经费来源约束条件，2->运行时决定
				@fundSrcLimite xml,	--静态定义的经费来源约束条件，
									--使用xml存储的指标：N'<root>
															--	<row id="1">
															--		<fCode>1</fCode>
															--		<fName>教育事业费</fName>
															...
															--</root>'
				--统计指标：
				@useMethod int,		--使用指标的方式：0->不使用，1->静态使用，具体分类见指标字段，2->运行时决定
				@typeCodeQuota xml,	--静态定义的设备分类统计指标，
									--使用xml存储的指标：N'<root>
														--	<row id="1">
														--		<eTypeCode>05010549</eTypeCode>
														--		<eTypeName>激光打印机</eTypeName>
														...
														--	...
														--</root>'
				@particleSize int,		--分组统计指标:1->院部，2->使用单位
				@moneySectionQuota xml,	--静态定义的金额分段统计指标，
										--使用xml存储的指标：N'<root>
															--	<row id="1">
															--		<startMoney>0</startMoney>
															--		<endMoney>10000</endMoney>
				...
				@appDirQuota xml,		--静态定义的使用方向统计指标，
										--使用xml存储的指标：N'<root>
															...
															--	...
															--</root>'
				@fundSrcQuota xml,		--静态定义的经费来源统计指标，
										--使用xml存储的指标：N'<root>
																--	<row id="1">
																--		<fCode>1</fCode>
																--		<fName>教育事业费</fName>
	...
	UpdateDate:modi by lw 2011-2-19删除统计范围和分组依据两个条件
			   ...
			   modi by lw 2012-11-01采用新的库结构重写
*/
...
AS
	...
	insert statisticTemplate(templateType, templateName, templateDesc, 
						typeCodeIsUsed, typeCodeLimite,
						moneySectionUsed, moneySectionLimiteOpr1, moneySectionLimiteData1,
						moneySectionLimiteOpr2, moneySectionLimiteData2,
						appDirUsed, appDirLimite, fundSrcUsed, fundSrcLimite,
						useMethod, typeCodeQuota, particleSize, 
						moneySectionQuota, appDirQuota, fundSrcQuota,
						lockManID, 	createManID, createManName, createTime) 
...
AS
	...
	set templateType = @templateType, templateName = @templateName, templateDesc = @templateDesc, 
		--约束条件：
		typeCodeIsUsed = @typeCodeIsUsed, typeCodeLimite = @typeCodeLimite,
		moneySectionUsed = @moneySectionUsed, 
		moneySectionLimiteOpr1 = @moneySectionLimiteOpr1, moneySectionLimiteData1 = @moneySectionLimiteData1,
		moneySectionLimiteOpr2 = @moneySectionLimiteOpr2, moneySectionLimiteData2 = @moneySectionLimiteData2,
		appDirUsed = @appDirUsed, appDirLimite = @appDirLimite, 
		fundSrcUsed = @fundSrcUsed, fundSrcLimite = @fundSrcLimite,
		--统计指标：
		useMethod = @useMethod, typeCodeQuota = @typeCodeQuota, particleSize = @particleSize, 
		moneySectionQuota = @moneySectionQuota, appDirQuota = @appDirQuota, fundSrcQuota = @fundSrcQuota,
		--维护情况:
...
CREATE TABLE [dbo].[totalCenter](
	...
	fundSrcDetail xml null,		--使用xml存储的指标：N'<root>
														...
														--	...
														--</root>'
	strWhere varchar(4000) null,	--统计条件构造的where子句：add by lw 2012-11-15
