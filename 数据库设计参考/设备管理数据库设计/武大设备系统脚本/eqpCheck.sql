...
--1.设备的清查（盘点）信息：
--1.1设备清查任务（盘点）表：
select * from eqpCheckInfo
alter TABLE eqpCheckInfo add	eqpDate smalldatetime null		--设备状态变化截止日期（设备日期）：add by lw 2013-5-27
select * from eqpCheckInfo
select * from eqpCheckDetailInfo where checkNum='201310310012'
update eqpCheckDetailInfo 
set lockKeeper=2
where checkNum='201310310012' and eCode='09010973'
select * from userInfo where cName='刘焱清'
select * from userInfo where jobNumber='00009284'
update userInfo 
set sysPassword='9369A71B2CA05876A068A505462AD673'
where jobNumber='00011279'

select * from userInfo where cName='毕卫民'

select * 
from eqpCheckDetailInfo ch left join equipmentList e on ch.eCode = e.eCode 
...
where  ch.eCode in (select c.eCode from eqpCheckDetailInfo c left join equipmentList e on c.eCode = e.eCode where  checkNum='201310310012')

select datediff(day, expCompleteDate,getdate()), checkNum, convert(varchar(10),applyDate,120) applyDate, applyState,
clgCode, clgName, uCode, uName, checkerID, checker, verifyManID, verifyMan, convert(varchar(10),verifyDate,120) verifyDate,
eqpNumber, eqpCheckedNumber, eqpWaitCheckNumber, eqpNoKeeperNumber, eqpNotExistNumber, 
eqpNeedMaintainNumber, eqpNeedScrapNumber, eqpNormalNumber, notes, eqpUploadedPhotoNumber
from eqpCheckInfo
where applyState=0 and datediff(day, expCompleteDate,getdate())<3 
order by expCompleteDate

select datediff(day,getdate(), expCompleteDate) leftDays, checkNum, convert(varchar(10),applyDate,120) applyDate, 
CONVERT(varchar(10),expCompleteDate, 120) expCompleteDate,
applyState,clgCode, clgName, uCode, uName, 
checkerID, checker, verifyManID, verifyMan, convert(varchar(10),verifyDate,120) verifyDate,eqpNumber, 
eqpCheckedNumber, eqpWaitCheckNumber, eqpNoKeeperNumber, eqpNotExistNumber, eqpNeedMaintainNumber, eqpNeedScrapNumber, 
eqpNormalNumber, notes, eqpUploadedPhotoNumber 
from eqpCheckInfo 
where applyState=0 and datediff(day,getdate(), expCompleteDate)<=3 
and clgCode='000' and (isnull(uCode,'') ='' or uCode='00000')
order by expCompleteDate

drop TABLE eqpCheckInfo
CREATE TABLE eqpCheckInfo(
	checkNum char(12) not null,			--主键：设备清查（盘点）号,使用4号号码发生器产生
											--8位日期代码+4位流水号
	applyDate smalldatetime not null,	--清查（盘点）开始时间
	...
	eqpDate smalldatetime null,			--设备状态变化截止日期（设备日期）：add by lw 2013-5-27
	
	checkerID varchar(10) not null,		--清查负责人（盘点人）工号
	checker nvarchar(30) not null,		--清查负责人（盘点人）
 ...
 CONSTRAINT [PK_eqpCheckInfo] PRIMARY KEY CLUSTERED 
(
	[checkNum] ASC
...
--索引：
--盘点开始日期索引：
CREATE NONCLUSTERED INDEX [IX_eqpCheckInfo] ON [dbo].[eqpCheckInfo] 
(
	[applyDate] ASC
)WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF) ON [PRIMARY]
--盘点生效日期索引：
CREATE NONCLUSTERED INDEX [IX_eqpCheckInfo_1] ON [dbo].[eqpCheckInfo] 
(
	[verifyDate] ASC
)WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF) ON [PRIMARY]
--盘点人索引：
CREATE NONCLUSTERED INDEX [IX_eqpCheckInfo_2] ON [dbo].[eqpCheckInfo] 
(
	[checkerID] ASC
)WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF) ON [PRIMARY]
--审核人索引：
CREATE NONCLUSTERED INDEX [IX_eqpCheckInfo_3] ON [dbo].[eqpCheckInfo] 
(
	[verifyManID] ASC
)WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF) ON [PRIMARY]
...
--1.2设备清查明细表：
alter TABLE eqpCheckDetailInfo add	oldKeeperID varchar(10) null		--原保管人工号
alter TABLE eqpCheckDetailInfo add	oldKeeper nvarchar(30) null		--原保管人
alter TABLE eqpCheckDetailInfo add	oldEqpLocate nvarchar(100) null	--原设备所在地址
alter TABLE eqpCheckDetailInfo add	oldEqpStatus char(1) null		--根据报表的规定重新定义设备的现状代码含义：本字段由第2号代码字典定义 modi by lw 2011-1-26
update eqpCheckDetailInfo
set oldEqpStatus = e.curEqpStatus,
	oldKeeperID = e.keeperID, oldKeeper = e.keeper, oldEqpLocate = e.eqpLocate
from eqpCheckDetailInfo c inner join equipmentList e on c.eCode=e.eCode
alter TABLE eqpCheckDetailInfo alter column	oldEqpStatus char(1) not null		--根据报表的规定重新定义设备的现状代码含义：本字段由第2号代码字典定义 modi by lw 2011-1-26

select * from eqpCheckDetailInfo
drop TABLE eqpCheckDetailInfo
CREATE TABLE eqpCheckDetailInfo(
	...
	--原设备保管信息：add by lw 2013-5-27
	oldKeeperID varchar(10) null,		--原保管人工号
	oldKeeper nvarchar(30) null,		--原保管人
	oldEqpLocate nvarchar(100) null,	--原设备所在地址
	oldEqpStatus char(1) not null,		--根据报表的规定重新定义设备的现状代码含义：本字段由第2号代码字典定义 modi by lw 2011-1-26
										--1：在用，指正在使用的仪器设备；
	...
	--编辑锁定人：
	lockManID varchar(10),				--当前正在锁定编辑的人工号
 CONSTRAINT [PK_eqpCheckDetailInfo] PRIMARY KEY CLUSTERED 
(
	[checkNum] ASC,
	[eCode] ASC
)WITH (IGNORE_DUP_KEY = OFF) ON [PRIMARY]
...
--外键：
ALTER TABLE [dbo].[eqpCheckDetailInfo] WITH CHECK ADD CONSTRAINT [FK_eqpCheckDetailInfo_eqpCheckInfo] FOREIGN KEY([checkNum])
REFERENCES [dbo].[eqpCheckInfo] ([checkNum])
ON UPDATE CASCADE
ON DELETE CASCADE
GO
ALTER TABLE [dbo].[eqpCheckDetailInfo] CHECK CONSTRAINT [FK_eqpCheckDetailInfo_eqpCheckInfo]
GO
--索引：
--盘点日期索引：
CREATE NONCLUSTERED INDEX [IX_eqpCheckDetailInfo_1] ON [dbo].[eqpCheckDetailInfo]
(
	[checkDate] ASC
)WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF) ON [PRIMARY]
--保管人索引：
CREATE NONCLUSTERED INDEX [IX_eqpCheckDetailInfo_2] ON [dbo].[eqpCheckDetailInfo]
(
	[keeperID] ASC
)WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF) ON [PRIMARY]
...
REFERENCES [dbo].[eqpCheckDetailInfo] ([checkNum], [eCode])
ON UPDATE CASCADE
ON DELETE CASCADE
GO
...
/*
	...
	input: 
				...
				@eqpDate varchar(10),		--设备状态变化截止日期（设备日期）：add by lw 2013-5-27
	...
	output: 
				@Ret		int output		--操作成功标识
							0:成功，1：要锁定的清查设备正在被别人编辑，
							5：要锁定清查的设备有一个未完成的清查任务，
							9:未知错误
	...
	UpdateDate: 增加使用单位名称 modi by lw 2012-5-4
				...
				根据方堃要求：1.盘点时的设备如果被调拨、报废等长事务锁锁定，应该允许盘点，所以使用叠加盘点事务锁方式设计锁；
							  2.盘点可以设定设备状态变化截止日期，主要是验收日期和报废日期
...
AS
	...
	--格式化设备状态变化截止日期：
	if (@eqpDate='')
		set @eqpDate = convert(varchar(10), getdate(), 120)
	declare @eqpDateFmt smalldatetime
	set @eqpDateFmt = DATEADD(day,1, convert(smalldatetime,@eqpDate,120))
	set @eqpDate = convert(varchar(10), @eqpDateFmt, 120)
	...
	set @count = (select COUNT(*) from equipmentList
					where clgCode = @clgCode and (@uCode='' or uCode = @uCode) 
						  --and acceptDate <@eqpDateFmt and (curEqpStatus not in('6','7') or scrapDate>=@eqpDateFmt)
						  --设备处要求将后期报废设备踢出：
						  and acceptDate <@eqpDateFmt and curEqpStatus not in('6','7')
	...
	--检查清查锁定的设备是否有清查任务长事务锁：
	set @count = (select COUNT(*) from equipmentList
					where clgCode = @clgCode and (@uCode='' or uCode = @uCode) 
						  --and acceptDate <@eqpDateFmt and (curEqpStatus not in('6','7') or scrapDate>=@eqpDateFmt)
						  --设备处要求将后期报废设备踢出：
						  and acceptDate <@eqpDateFmt and curEqpStatus not in('6','7')
						  and lock4LongTime >= 10)
	if (@count>0)
	begin
		set @Ret = 5
		return
	...
	begin tran
		insert eqpCheckInfo(checkNum, applyDate, expCompleteDate, clgCode, clgName, uCode, uName, eqpDate, checkerID, checker, 
							staffNumOfLocation, notes,
							lockManID, modiManID, modiManName, modiTime) 
		values (@checkNum, @applyDate, @expCompleteDate, @clgCode, @clgName, @uCode, @uName, @eqpDateFmt, @checkerID, @checker, 
							@staffNumOfLocation, @notes,
		...
		--插入盘点表设备明细：
		insert eqpCheckDetailInfo(checkNum, eCode, eName, checkStatus, keeperID, keeper, 
								oldKeeperID, oldKeeper, oldEqpLocate, oldEqpStatus)
		select @checkNum, eCode, eName, 0, keeperID, keeper,
				keeperID, keeper,eqpLocate, curEqpStatus
		from equipmentList
		where clgCode = @clgCode and (@uCode='' or uCode = @uCode) 
				--and acceptDate <@eqpDateFmt and (curEqpStatus not in('6','7') or scrapDate>=@eqpDateFmt)
				--设备处要求将后期报废设备踢出：
		...
		set lock4LongTime = isnull(lock4LongTime,0)+10, bakLInvoiceNum =lInvoiceNum , lInvoiceNum = @checkNum
...
select * from eqpCheckInfo where checkNum='201207280006'
select * from eqpCheckDetailInfo where checkNum='201207280006'
...
AS
	...
	set @count=(select count(*) from eqpCheckInfo where checkNum = @checkNum)	
	if (@count = 0)	--不存在
	begin
		set @Ret = 1
		return
	...
	select @clgCode= clgCode, @uCode = uCode from eqpCheckInfo where checkNum = @checkNum
	
	--计算保管人锁定状态：
	--逐个扫描清查设备，如果没有保管人ID，则使用姓名自动匹配保管人
	declare @eCode char(8), @keeperID varchar(10), @keeper nvarchar(30)	--设备编号，保管人工号，保管人
	declare tar cursor for
	select eCode, keeperID, keeper from eqpCheckDetailInfo
...
select * from eqpCheckDetailInfo where checkNum = '201207270001'
...
AS
	...
	set @eqpNumber = (select count(*) from eqpCheckDetailInfo where checkNum = @checkNum)
	declare @eqpCheckedNumber int, @eqpWaitCheckNumber int, @eqpNoKeeperNumber int
	set @eqpCheckedNumber = (select count(*) from eqpCheckDetailInfo where checkNum = @checkNum and lockKeeper = 9)		--已经确认过的设备数量
	set @eqpWaitCheckNumber = (select count(*) from eqpCheckDetailInfo where checkNum = @checkNum and lockKeeper in(1,2))	--待确认设备数量
	set @eqpNoKeeperNumber = (select count(*) from eqpCheckDetailInfo where checkNum = @checkNum and lockKeeper = 0)	--没有保管人的设备数量
	
	declare @eqpUploadedPhotoNumber int	--已上传照片的设备数量
	set @eqpUploadedPhotoNumber = (select count(*) from eqpCheckDetailInfo 
	...
	set @eqpNormalNumber = (select count(*) from eqpCheckDetailInfo where checkNum = @checkNum and checkStatus = '1')
	declare @eqpNeedMaintainNumber int	--明确为“待修”状态的设备
	set @eqpNeedMaintainNumber = (select count(*) from eqpCheckDetailInfo where checkNum = @checkNum and checkStatus = '3')
	declare @eqpNeedScrapNumber int		--明确为“待报废”状态的设备
	set @eqpNeedScrapNumber = (select count(*) from eqpCheckDetailInfo where checkNum = @checkNum and checkStatus = '4')
	declare @eqpNotExistNumber int		--明确为“有帐无物、无帐无物”状态的设备
	set @eqpNeedScrapNumber = (select count(*) from eqpCheckDetailInfo where checkNum = @checkNum and checkStatus in('5','6'))
...
drop PROCEDURE exceedingCheckApply
/*
	name:		exceedingCheckApply
	function:	1.3.将指定的盘点单展期
	input: 
				@checkNum char(12),			--设备清查单号
				@expCompleteDate varchar(10),	--新的预计完成时间
				@modiManID varchar(10) output,	--修改人，如果当前清查（盘点）单正在被人占用编辑则返回该人的工号
	output: 
				@Ret		int output		--操作成功标识
							0:成功，1：指定的清查单不存在，2：指定的清查单正被别人锁定，3：指定的清查单已经结束，9：未知错误
	author:		卢苇
	CreateDate:	2013-10-30
	UpdateDate: 

*/
create PROCEDURE exceedingCheckApply
	@checkNum char(12),				--设备清查单号
	@expCompleteDate varchar(10),	--新的预计完成时间
	@modiManID varchar(10) output,	--修改人，如果当前清查（盘点）单正在被人占用编辑则返回该人的工号
	@Ret		int output			--操作成功标识	
...
AS
	...
	set @thisLockMan = isnull((select lockManID from eqpCheckInfo where checkNum = @checkNum),'')
	...
	set @applyState = (select applyState from eqpCheckInfo where checkNum = @checkNum)
	if (@applyState <> 0)
	begin
		set @Ret = 3
		return
	...
	update eqpCheckInfo
	set expCompleteDate = @expCompleteDate, modiManID = @modiManID, modiManName=@modiManName, modiTime=@updateTime
	where checkNum = @checkNum

	--登记工作日志：
	insert workNote(userID, userName, actionTime, actions, actionObject)
	values(@modiManID, @modiManName, @updateTime, '生成清查单', '系统根据用户' + @modiManName + 
					'的要求将设备清查单[' + @checkNum + ']的预计完成日期修改为：'+@expCompleteDate+'。')
	set @Ret = 0
GO
...
AS
	set @Ret = 9
	set @lockManID = isnull((select lockManID from eqpCheckInfo where checkNum = @checkNum),'')
	set @Ret = 0
GO
...
AS
	...
	begin
		...
		update eqpCheckInfo set lockManID = '' where checkNum = @checkNum
	end
	else
	begin
		set @Ret = 0
...
AS
	set @Ret = 9
	set @lockManID = isnull((select lockManID from eqpCheckDetailInfo where checkNum = @checkNum and eCode = @eCode),'')
	set @Ret = 0
GO
...
AS
	...
	--判断要锁定的清查设备是否存在
	declare @count as int
	set @count=(select count(*) from eqpCheckDetailInfo where checkNum = @checkNum and eCode = @eCode)	
	if (@count = 0)	--不存在
	begin
		set @Ret = 1
		return
	...
	--检查编辑锁：
	declare @thisLockMan varchar(10)
	set @thisLockMan = isnull((select lockManID from eqpCheckDetailInfo where checkNum = @checkNum and eCode = @eCode),'')
	if (@thisLockMan <> '' and @thisLockMan <> @lockManID)
	begin
		set @lockManID = @thisLockMan
		set @Ret = 2
...
AS
	...
	begin
		...
		update eqpCheckDetailInfo set lockManID = '' where checkNum = @checkNum and eCode = @eCode
	end
	else
	begin
		set @Ret = 0
...
AS
	...
	declare @thisLockMan varchar(10)
	select @thisLockMan = isnull(lockManID,'') from eqpCheckDetailInfo where checkNum = @checkNum and eCode = @eCode
	--检查编辑锁：
	if (@thisLockMan <> '' and @thisLockMan <> @modiManID)
	begin
		set @modiManID = @thisLockMan
	...
	declare @applyState int
	select @applyState = applyState from eqpCheckInfo where checkNum = @checkNum
	--检查单据状态:
	if (@applyState <> 0)
	begin
		set @Ret = 3
...
/*
	...
	UpdateDate: modi by lw 2012-7-28 增加长事务锁处理
				modi by lw 2013-5-27修改设备清查事务锁为叠加方式！
*/
...
AS
	...
	begin tran
		--撤销设备的长事务锁：
		update equipmentList
		set lock4LongTime = lock4LongTime-10, lInvoiceNum = bakLInvoiceNum
		where eCode in (select eCode from eqpCheckDetailInfo where checkNum = @checkNum)
		if @@ERROR <> 0 
		begin
			set @Ret = 9
			rollback tran
		...
		--删除清查表：
		delete eqpCheckInfo where checkNum = @checkNum
	commit tran
...
/*
	...
	UpdateDate: modi by lw 2012-7-28
				...
				2.将作废与生效分离
				modi by lw 2013-5-27
				1.修改设备清查事务锁为叠加方式！
				2.修订设备清查只修改设备保管人和设备所在地，不改变设备隶属院部和单位！
*/
...
AS
	...
	--检查清查设备编辑锁：
	select @count = (select COUNT(*) from eqpCheckDetailInfo where checkNum = @checkNum and isnull(lockManID,'') <> '')
	if (@count > 0)
	begin
		set @Ret = 3
		return
	...
	begin tran
		...
		begin
			...
			begin
				--获取设备保管人的隶属院部和使用单位信息：del by lw 2013-5-27不修改设备隶属院部和单位
				--declare @clgCode char(3), @uCode varchar(8)		--学院代码/使用单位代码
				...
				select @oldClgCode = isnull(clgCode,''), @oldUCode = isnull(uCode,''), 
						@oldKeeperID = isnull(keeperID,''), @oldEqpLocate = isnull(eqpLocate,'')
				from equipmentList
				where eCode = @eCode
				...
				if (@keeperID <> @oldKeeperID or @eqpLocate <> @oldEqpLocate)
				begin
					--登记设备生命周期表:
					...
					select @eCode, e.eName, e.eModel, e.eFormat, @oldClgCode, clg.clgName, @oldUCode, u.uName, 
						@keeperID, @keeper, @eqpLocate,
						e.annexAmount, e.price, e.totalAmount,
						c.checkDate, '清查设备时变更了该设备的保管信息',
						'清查',@checkNum
					from equipmentList e inner join eqpCheckDetailInfo c on e.eCode = c.eCode
						left join college clg on clg.clgCode = @oldClgCode
						left join useUnit u on u.clgCode = @oldClgCode and u.uCode = @oldUCode
...
/*
	...
	output: 
				...
				@Ret		int output				--操作成功标识
													...
													3：该单据不是已生效状态，
													4：要撤销生效的清查单中的设备正被别人编辑锁定，
													5：该清查单中的设备被另一个清查任务锁定，不能撤销！
													6：该清查单中的设备已经变动，不能撤销！
													9：未知错误
	author:		卢苇
	CreateDate:	2011-11-22
...
AS
	...
	begin tran
		...
		declare @eCode char(8)	--设备清查单号/设备编号
		declare @oldKeeperID varchar(10), @oldKeeper nvarchar(30)		--原保管人
		declare @oldEqpLocate nvarchar(100)	--原设备所在地址
		declare @oldEqpStatus char(1)		--原设备的现状代码
		declare tar cursor for
		select eCode, oldKeeperID, oldKeeper, oldEqpLocate, oldEqpStatus 
		from eqpCheckDetailInfo where checkNum = @checkNum
		OPEN tar
		FETCH NEXT FROM tar INTO @eCode, @oldKeeperID, @oldKeeper, @oldEqpLocate, @oldEqpStatus
		WHILE @@FETCH_STATUS = 0
		begin
			...
			begin
				...
				--恢复设备清查前的状态：
				update equipmentList
				set curEqpStatus = @oldEqpStatus,
					keeperID = @oldKeeperID, keeper = @oldKeeper,
					eqpLocate = @oldEqpLocate
				where eCode = @eCode
				if @@ERROR <> 0 
				begin
					set @Ret = 9
...
GO
--测试：
declare	@cancelDate smalldatetime--取消日期
declare	@Ret		int --操作成功标识
exec dbo.cancelVerifyCheck '201305270021', '0000000000',@cancelDate output, @Ret output
select @Ret


select * from eqpCheckInfo
select * from eqpLifeCycle where eCode in (select eCode from eqpCheckDetailInfo where checkNum= '201305270021')

update eqpCheckInfo
set applyState=0, lockManID = ''
...
/*
	...
	UpdateDate: modi by lw 2013-5-27修改设备清查事务锁为叠加方式！
*/
...
from 
 eqpCheckDetailInfo ch left join equipmentList e on ch.eCode = e.eCode 
	left join country c on e.cCode = c.cCode 
	left join fundSrc f on e.fCode = f.fCode 
	left join appDir a on e.aCode = a.aCode 
	left join college clg on e.clgCode = clg.clgCode 
...
select * from eqpCheckDetailInfo where checkNum='201305270003'
select * from equipmentList where eCode in (select eCode from eqpCheckDetailInfo where checkNum='201305270003')
