use hustOA
select * from userInfo where sysPassword='C4DEADD49541FE5E8CC2F9461A3D1607'
select e.eName, * from eqpCheckDetailInfo c left join equipmentList e on c.eCode = e.eCode where  c.checkNum='201207310001' and e.eName like '笔记本%'
select * from equipmentList
select * from eqpCheckDetailInfo c left join equipList e on c.eCode = e.eCode where  c.checkNum='201207310001' and e.eName like '笔记本%'
...
--增加短信管理：add by lw 2012-8-19
insert sysRight(rightName, rightEName, Url, rightType, rightKind, rightClass, rightItem, canUse, rightDesc)
values('短信管理','SMSList','SMSManager/SMSList.html','D',8,10,0,'Y','可以新建、修改、删除、发送、查看和导出短信的权利。')
...
drop table t1
select rightKind,rightClass,rightItem,oprType,oprName,oprEName,oprDesc,sortNum from t1
insert sysDataOpr(rightKind,rightClass,rightItem,oprType,oprName,oprEName,oprDesc,sortNum)
select rightKind,rightClass,rightItem,oprType,oprName,oprEName,oprDesc,sortNum from t1
where convert(varchar(5),rightKind*1000+rightClass*100+rightItem)+oprEName not in (
	select convert(varchar(5),rightKind*1000+rightClass*100+rightItem)+oprEName from sysDataOpr
	)
select * from sysDataOpr where rightKind=8

--增加系统资源：add by lw 2011-11-20
SELECT * FROM sysRight where rightKind = 3 and rightClass = 6
delete sysRight where rightKind = 3 and rightClass = 6
insert sysRight(rightName, rightEName, Url, rightType, rightKind, rightClass, rightItem, 
				...
				rightDesc)
values('设备清查表','eqpCheckInfoList','eqpCheck/eqpCheckInfoList.html','D',3,6,0,
				'Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','可以查询、删除、导出设备清查表和审计的权利。')
...
from sysDataOpr
SELECT * FROM sysDataOpr
where rightKind = 3 and rightClass = 6 and rightItem = 0 and oprType = 2
--增加的操作集：
insert sysDataOpr
...
insert sysDataOpr
values(7,4,0,3,'打印卡片','cmdPrintCard','打印用户卡片的权限')
insert sysDataOpr
values(7,4,0,3,'打印','cmdPrint','打印用户一览表的权限')
insert sysDataOpr
values(7,4,0,3,'导出','cmdOutput','导出用户一览表的权限')
...
set oprName = '清查',
	...
	oprDesc = '可以新建、删除、生效、查询、导出设备清查单和指派设备保管人、认领设备的权利。'
where rightKind = 3 and rightClass = 1 and rightItem = 0 and oprType = 2 and oprEName='cmdChecking'

SELECT * FROM sysDataOpr
where rightKind = 3 and rightClass = 6 and rightItem = 0 

delete sysDataOpr
where rightKind = 3 and rightClass = 6 and rightItem = 0 

insert sysDataOpr
values(3,6,0,1,'默认列表','cmdDefaultList','设备清查单默认列表功能')
insert sysDataOpr
values(3,6,0,1,'筛选','cmdFilter','设定查询条件，约束数据列表的显示范围')
insert sysDataOpr
values(3,6,0,1,'查阅卡片','cmdShowCard','查看指定清查单的详细信息')
insert sysDataOpr
values(3,6,0,1,'显示清查设备','cmdCheckEqps','显示设备清查单的清查设备')
insert sysDataOpr
values(3,6,0,1,'统计','cmdCalc','统计指定清查表的清查情况')
insert sysDataOpr
values(3,6,0,2,'新建','cmdNew','根据指定清查范围生成要清查的设备清单')
insert sysDataOpr
values(3,6,0,2,'删除','cmdDel','删除指定行的设备清查表')
insert sysDataOpr
values(3,6,0,3,'打印卡片','cmdPrintCard','打印指定的设备清查单详细信息')
insert sysDataOpr
values(3,6,0,3,'打印（列表）','cmdPrint','打印清查单列表')
insert sysDataOpr
values(3,6,0,3,'导出清查单','cmdOutputCard','导出指定的设备清查单详细信息')
insert sysDataOpr
values(3,6,0,3,'导出','cmdOutput','导出当前数据窗口中的列表')
insert sysDataOpr
values(3,6,0,4,'生效','cmdVerify','将指定的设备清查单生效')
insert sysDataOpr
values(3,6,0,4,'作废','cmdInvalid','将指定的设备清查单作废')
insert sysDataOpr
values(3,6,0,4,'取消生效','cmdCancelVerify','取消已经生效或作废的设备清查单')

insert sysDataOpr
values(3,6,0,4,'指派保管人','cmdDefineKeeper','指定设备保管人')
...
insert sysRoleRight(sysRoleID,
	rightName, rightEName, Url, rightType, rightKind, rightClass, rightItem, rightDesc)
select 2, rightName, rightEName, Url, rightType, rightKind, rightClass, rightItem, rightDesc 
from sysRight
...
insert sysRoleRight(sysRoleID,
	rightName, rightEName, Url, rightType, rightKind, rightClass, rightItem, rightDesc)
select 3, rightName, rightEName, Url, rightType, rightKind, rightClass, rightItem, rightDesc 
from sysRight

--预定义的角色：单位系统管理员，拥有单位的全部权限，可以定义本单位的特定角色：
insert sysRoleRight(sysRoleID,
	rightName, rightEName, Url, rightType, rightKind, rightClass, rightItem, rightDesc)
select 4, rightName, rightEName, Url, rightType, rightKind, rightClass, rightItem, rightDesc 
from sysRight
...
select 1,sortNum, rightKind, rightClass, rightItem, oprType, oprName, oprEName, oprDesc, 4
from sysDataOpr
...
values('0000000000',1,'超级用户','001','脉冲强磁场科学中心', 'administrator','D63F7AB0AA4546EC668C1D11EB795819','0000000000')
--初始密码是''
...
use epdc2
select * from userInfo
delete userInfo where sysuserName<>'bwm'

select * from eqpAcceptInfo where applyDate<'20111231' and applyDate>'20110101'
select * from equipmentList where acceptApplyID in (select acceptapplyid from  eqpAcceptInfo where applyDate<'20111231' and applyDate>'20110101')

select * from  eqpAcceptInfo where applyDate>'20111231' or applyDate<'20110101'

delete eqpAcceptInfo where applyDate>'20111231' or applyDate<'20110101'
delete eqpAcceptInfo where left(convert(varchar(10),applyDate,120),7) in ('2011-02','2011-04','2011-06','2011-08','2011-09')

select * from equipmentList
delete equipmentList where acceptApplyID is null or acceptApplyID not in (select acceptapplyid from  eqpAcceptInfo)

select * from eqpAllocation 
select * from equipmentAllocationDetail where eCode not in (select eCode from equipmentList)

select * from equipmentAllocation where alcNum in 
(select alcNum from equipmentAllocationDetail where eCode not in (select eCode from equipmentList))


delete equipmentAllocation where alcNum in (select alcNum from equipmentAllocationDetail where eCode not in (select eCode from equipmentList))

select * from equipmentScrap where scrapNum in 
(
select scrapNum from sEquipmentScrapDetail where eCode not in (select eCode from equipmentList)
UNION all
select scrapNum from bEquipmentScrapDetail where eCode not in (select eCode from equipmentList)
)

delete equipmentScrap where scrapNum in 
(
select scrapNum from sEquipmentScrapDetail where eCode not in (select eCode from equipmentList)
UNION all
select scrapNum from bEquipmentScrapDetail where eCode not in (select eCode from equipmentList)
...
delete codeDictionary where classCode = 1
...
select * from sysDataOpr where rightkind = 3 and rightclass = 6
delete sysDataOpr where rightkind = 3 and rightclass = 6
...
insert sysDataOpr
values(3,6,0,1,'查阅盘点表','cmdShowCard','查看指定盘点表的设备详细信息')

insert sysDataOpr
values(3,6,0,1,'盘盈盘亏','cmdCalc','统计指定盘点表的盘盈盘亏情况')

insert sysDataOpr
values(3,6,0,2,'新建','cmdNew','根据指定单位生成要盘点的设备清单')

insert sysDataOpr
values(3,6,0,2,'继续盘点','cmdCheck','继续盘点设备的权属、状态')

insert sysDataOpr
values(3,6,0,2,'删除','cmdDel','删除指定行的盘点表')

insert sysDataOpr
values(3,6,0,3,'打印盘点表','cmdPrintCard','打印指定的盘点表详细信息')

insert sysDataOpr
values(3,6,0,3,'打印（列表）','cmdPrint','打印盘点列表')

insert sysDataOpr
values(3,6,0,3,'导出盘点表','cmdOutputCard','导出指定的盘点表详细信息')

insert sysDataOpr
values(3,6,0,3,'导出列表','cmdOutput','导出当前数据窗口中的列表')

insert sysDataOpr
values(3,6,0,4,'生效','cmdVerify','将盘点信息生效')

insert sysDataOpr
values(3,6,0,4,'作废','cmdInvalid','将指定的盘点表作废')
...
--统计系统正式运行的数据：
select * from eqpAcceptInfo where acceptDate > convert(smalldatetime, '2011-10-08',120)
--验收单合计：3323
select * from equipmentList where acceptDate > convert(smalldatetime, '2011-10-08',120)
select * from equipmentList
--新增设备合计：8080
--设备总数：160585
select * from equipmentAllocation where alcDate > convert(smalldatetime, '2011-10-08',120)
select count(*) from equipmentAllocation e left join equipmentAllocationDetail d on e.alcNum=d.alcNum
where e.alcDate > convert(smalldatetime, '2011-10-08',120)
--调拨单合计：4115, 4135台套
select * from equipmentScrap where scrapDate > convert(smalldatetime, '2011-10-08',120)
select count(*) from equipmentScrap e left join sEquipmentScrapDetail d on e.scrapNum=d.scrapNum
where e.scrapDate > convert(smalldatetime, '2011-10-08',120)
select * from equipmentScrap e right join bEquipmentScrapDetail d on e.scrapNum=d.scrapNum
where e.scrapDate > convert(smalldatetime, '2011-10-08',120)
select * from equipmentScrap
where scrapDate > convert(smalldatetime, '2011-10-08',120) and isBigEquipment = 1
--报废单合计：121，
--小型设备：1289台套，大型设备：31台套
--在线用户数：
select * from activeUsers

select distinct userID 
from 
(select userID from workNote
union all
select userID from workNoteBak
) as tab
--使用过系统的用户数：514
select * from userInfo
--可登陆用户数：6527
...
select * from useUnit
--使用单位总数：516
select * from workNoteBak where actions not in ('登录','登出')

select (char(39) + w.userID) as dd , w.userName, u.clgName, isnull(u.uName,''), count(*) from workNoteBak w left join userInfo u on w.userID=u.jobNumber
where actions not in ('登录','登出')
group by w.userID,w.userName, u.clgName, u.uName
order by count(*) desc

--剔除登入和登出后对数据的操纵次数：14503
