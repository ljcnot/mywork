...
/*
	...
	UpdateDate: 2013-06-21根据客户要求支持投票和评论，支持定向发布到指定人员
*/
...
--学术报告与公告的联合查询语句：
select 2 type, 1 orderNum, aReportID, convert(varchar(10),publishTime,120) publishTime, 
	(convert(varchar(19),reportStartTime,120) + ' ' + reportMan + '在' + reportPlace + '作“' + topic + '”') title
from academicReports
where isPublish = 1 and isOver = 0
union all
select 1, orderNum, bulletinID, bulletinTime, bulletinTitle 
from bulletinInfo
where isActive =1 
order by type desc, orderNum, publishTime desc

select *, isnull(CONVERT(varchar(10),autoCloseDate,120),'') from bulletinInfo

update bulletinInfo
set autoCloseDate=null
where isActive=1

alter TABLE bulletinInfo add enableVote smallint default(0)			--是否允许投票：0->允许，1->不允许
update bulletinInfo
set enableVote=0
alter TABLE bulletinInfo add publishTo xml null					--发布对象（阅读权限）：
alter TABLE bulletinInfo add createrID varchar(10) null			--创建人工号
alter TABLE bulletinInfo add creater varchar(30) null			--创建人
update bulletinInfo
set createrID = modiManID, creater = modiManName

alter TABLE bulletinInfo add voteOption xml null					--投票选项
alter TABLE bulletinInfo add isClosed int default(0)				--是否关闭：0->未关闭，1->已关闭 add by lw 2013-9-8
alter TABLE bulletinInfo add closedTime smalldatetime null			--关闭日期 add by lw 2013-9-8
update bulletinInfo
set isClosed=0
alter TABLE bulletinInfo add onTop int default(0)					--是否置顶显示：0->不置顶，1->置顶显示
update bulletinInfo
set onTop=0
...
begin
	...
	--添加部门用户：
	set @count=(select @publishTo.exist('/root/unit'))
	if (@count>0)
	begin
		insert @selectedMan(userID, userCName)
		select u.jobNumber, u.cName 
		from userInfo u 
		where u.uCode in (SELECT A.unit.value('./@uCode','varchar(8)') from @publishTo.nodes('/root/unit') as A(unit))
		set @isAllEmpty = 'N'
	end
...
set publishTo=N'<root>'+
					'<user userID="G201300001" userCName="程远" />'+	--包含的用户
					'<unit uCode="001011" uName="科学实验部" />'+			--包含的部门
					--'<sysRole id="3" desc="场地管理员" />'+				--包含的角色
					--'<exceptUser userID="G201300003" userCName="杨斌" />'+			--除外人员
				'</root>'
...
select dbo.canSeeBulletinInfo('201306270005','G201300040')
...
set @publishTo=N'<root>
  ...
  <sysRole id="3" desc="场地管理员" />
  <sysRole id="4" desc="实验站管理员" />
  <sysRole id="5" desc="气体管理员" />
  <sysRole id="6" desc="设备管理员" />
  <sysRole id="7" desc="部长" />
  <sysRole id="96" desc="角色1" />
</root>'
...
set @publishTo=N'
<root>
  <unit uCode="001001" uName="办公室" />
  <unit uCode="001005" uName="磁体部" />
  <unit uCode="001006" uName="控制部" />
  <unit uCode="001009" uName="设备保障部" />
  <unit uCode="001010" uName="中心领导" />
  <unit uCode="001011" uName="科学实验部" />
  <unit uCode="001012" uName="电源部" />
</root>'
SELECT cast(A.col1.query('data(./@uCode)') as varchar(10)),
	cast(A.col1.query('data(./@uName)') as nvarchar(30))
from @publishTo.nodes('/root/unit') as A(col1)
...
--测试：
declare @ret int
exec dbo.addBulletinDiscuss '201311270017','','<p>hello!</p>','G201300052',@ret output
select @ret
